...
const estruturaCarousel = [
  ...
  },
  {
    ...
    label:
      "Serviço de ajuste no triclamp de 6” e de 2 ½” na lateral e topo dos contentores metálicos",
  },
  {
  ...
  },
  {
    img: "img/psvs_2.jfif",
    alt: "psvs 2",
    label: "Temos algumas com data de validade vencida (testes feitos em 2020)",
  },
  {
    img: "img/grade.jpg",
...
// .classList.replace("arrowDown", "arrowNormal")
const cabListas = document.querySelectorAll(".cabLista");

for (let i = 0; i < cabListas.length; i++) {
  const cablista = cabListas[i];
  cablista.addEventListener("click", (e) => {
    const arrow = e.composedPath()[0].firstChild;
    if (arrow.classList.contains("arrowNormal")) {
      arrow.classList.replace("arrowNormal", "arrowDown");
    } else {
      arrow.classList.replace("arrowDown", "arrowNormal");
    }
  });
}
